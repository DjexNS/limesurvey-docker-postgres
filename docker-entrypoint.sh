...
EOPHP
    ...
    then
        port=5432
        db=pgsql
        set_config 'charset' "'UTF8'"
    else 
        port=3306
    fi
    

    set_config 'connectionString' "'$db:host=$host;port=$port;dbname=$LIMESURVEY_DB_NAME;'"
...
EOPHP
	...
	DBSTATUS=$(TERM=dumb php -- "$LIMESURVEY_DB_HOST" "$LIMESURVEY_DB_USER" "$LIMESURVEY_DB_PASSWORD" "$LIMESURVEY_DB_NAME" "$LIMESURVEY_TABLE_PREFIX" "$MYSQL_SSL_CA" "$DB_TYPE" <<'EOPHP'
<?php
...
if ($argv[7] == 'POSTGRES') {
    ...
    fwrite($stderr, 'Testing PostgreSQL connection ...' . "\n");

    do {
        fwrite($stderr, "host=$host port=$port dbname=$argv[4] user=$argv[2] password=$argv[3]" . "\n");
        $db_handle = pg_connect("host=$host port=$port dbname=$argv[4] user=$argv[2] password=$argv[3]");

        if (!$db_handle) {
            fwrite($stderr, 'Connection attempt failed.' . "\n");
            if ($maxTries <= 0) {
                exit(1);
            }
            sleep(3);
    ...
    fwrite($stderr, 'Connection attempt succeeded.' . "\n");

    $query = "SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = '"
            . pg_escape_string($argv[5]) . "users')";
